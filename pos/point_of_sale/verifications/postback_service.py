...
def get_collect_user_info(package_id):
    get_collect_user_status = db_agent.execute_select_one_parameter(constants.COLLECT_USER_INFO_BY_PACKAGE, package_id)
    return get_collect_user_status['CollectUserInfo']
...
def verify_postback_url(action, package_id, trans_id):
    ...
    if action == "SignUp":
        ...
        for id in matched_postback_ids:
            ...
            if trans_type == 506:
                expected_stage = "Instant Conversion"
