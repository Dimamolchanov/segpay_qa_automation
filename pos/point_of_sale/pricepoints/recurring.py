...
def generate_scenario(scenario):
    ...
    try:
        ...
        rebill_date = f"NextDate: CurrentDate + {scenario['merchantbillconfig']['InitialLen']}"
        ...
        if scenario['type'] == 501:
            purch_type = 'PurchType: 501'
        elif scenario['type'] == 506:
            purch_type = 'PurchType: 506'
        if scenario['paypal']:
            payments = ['CC', 'PayPal']
        else:
            payments = ['CC']
        ...
        for i in payments: # Singup
            ...
            print(f"Assets:     | PurchStatus: 801 | {purch_type} | AuthCurrency: {d['dmc']} |  Purchases: 1 | CustAddress,CustCity,CustState,CustPhone: Blank | {rebill_date}")
        ...
        for i in payments:
            ...
            print('|Excptected Results |')
            print("--------------------------------------------------------------------------------------------------------------------------")
            print(f"Multitrans: | AuthCode:OK:0 | TransSource: 121 | TransStatus: 186 | TransType: 1011 | {pay_type} | MerchantCurrency: {d['dmc']} | Language: {d['lang']} |"
                  f"CustAddress,CustCity,CustState,CustPhone => Blank ")
            print(f"Assets:     | PurchStatus: 801 | {purch_type} | AuthCurrency: {d['dmc']} |  Purchases: 1 | CustAddress,CustCity,CustState,CustPhone: Blank | {rebill_date}")
            print(f"Email:      | PointOfSaleEmailQueue should  have email | EmailTypeID: 981  ")
            print(f"PostBacks   | {postbacks} ")
